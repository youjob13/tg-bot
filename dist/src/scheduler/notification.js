...
const pushNotification = async () => {
    try {
        ...
        const needNotificationRequests = getNonNullableValues(await Promise.all(needNotificationDates.map(async ([chatId, requestId]) => requestCollection.getApprovedRequestByChatIdAndRequestId(Number(chatId), requestId))));
        for (const request of needNotificationRequests) {
            const userNotificationPromise = bot.api.sendMessage(request.chatId, `Привет! Напоминаю, что Вы записаны завтра на сеанс!🤍 \n Дата: ${formatToDate(request.date)} `);
            const adminNotificationPromise = bot.api.sendMessage(ADMIN_ID, `Запись на завтра:\n ${request.userFullName}\n @${request.username}\n ${formatToDate(request.date)}\n ${request.userCustomData}`);
            const markDateAsNotifiedPromise = scheduleCollection.markDateIsNotified(`${request.chatId}|${request.requestId}`);
